    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-zinc-400">
        ...
        <span className="text-[11px] font-normal text-slate-400 dark:text-zinc-500">
          (Transforms text directly inside the input workbench)
        </span>
      </div>
      <div className="flex flex-wrap items-center gap-1.5">
        {quickOptions.map((opt) => (
          <button
            ...
            className="px-2.5 py-1 text-xs font-mono font-medium rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 hover:border-indigo-400 dark:hover:border-indigo-600 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/30 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-2xs"
          >
            {opt.label}
          </button>
        ))}
